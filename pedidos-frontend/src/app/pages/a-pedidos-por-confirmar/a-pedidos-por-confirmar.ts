...
export class APedidosPorConfirmar implements OnInit {
  ...
  goToEditOrderForConfirm(orderId?: number) {
    this.router.navigate([`/admin/editar-pedidos-por-confirmar/${orderId}`]);
  }
  ...
  descargarTabla() {
    // Tomamos la tabla directamente del DOM
        ...
        const nombreArchivo = `pedidos-por-confirmar-${new Date().toISOString().slice(0,10)}.xlsx`;
